...
export function CampaignsTab({ organization }) {
  const navigate = useNavigate()
  const [page, setPage] = useState(1)
  const limit = 10

  const { data, isLoading } = useAdminCampaigns({ organization: organization.id, page, page_size: limit })
  ...
  if (campaigns.length === 0) {
    return (
      <EmptyState
        ...
        description={`${organization.organization_name} hasn't started a campaign.`}
      />
    )
  }
