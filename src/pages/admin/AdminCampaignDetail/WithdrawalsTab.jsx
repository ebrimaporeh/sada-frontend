...
import { useAdminCampaignPayouts } from '@/hooks/usePayments'
...
export function WithdrawalsTab({ campaign }) {
  const { data: payouts, isLoading } = useAdminCampaignPayouts(campaign.id)
  const list = payouts ?? []
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-3 gap-3">
        <StatCard label="Withdrawals" value={list.length} icon={Banknote} color="bg-primary/10 text-primary" />
      ...
      {list.length === 0 ? (
        <EmptyState
          ...
          description="This campaign's owner hasn't requested a payout."
        />
      ) : (
                  ...
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {provMeta?.name || p.provider} · +220 {p.phone?.replace(/^\+220/, '')}
                      </p>
