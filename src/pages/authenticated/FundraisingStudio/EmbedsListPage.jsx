    ...
    <div>
      ...
      {embeds.length === 0 ? (
        ...
        <div className="space-y-3">
          {embeds.map((embed) => (
            <div key={embed.id} className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 p-4 rounded-xl border bg-card">
              {/* Row 1: icon + title -- always its own row on mobile so a
                  long name never fights the status/action buttons for
                  space; sm:+ this and row 2 sit side by side again. */}
