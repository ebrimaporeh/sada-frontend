...
export function FeaturedCampaigns() {
  const { featured, urgent } = useFeaturedCampaigns()

  return (
    <section className="max-w-7xl mx-auto px-4 sm:px-6 py-14 space-y-14">
      {/* Featured */}
      <div>
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-2xl sm:text-3xl font-bold flex items-center gap-2">
              <span className="text-red-500">🚨</span> Urgent Needs
            </h2>
            <p className="text-muted-foreground text-sm mt-1">These campaigns need your help most right now</p>
          </div>
        </div>
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {urgent.map((c) => (
